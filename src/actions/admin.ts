    ...
    getAppSettings: defineAction({
        handler: async (_, { locals }) => {
            ...
            try {
                ...
                const settingsStr = await (IMGBED_KV as any).get(APP_SETTINGS_KEY, { type: "text", consistency: "strong" });

                if (settingsStr) {
                    try {
                        settings = JSON.parse(settingsStr);
                    } catch (e) {
                        console.error(`Failed to parse app settings from KV (${APP_SETTINGS_KEY}):`, e);
                ...
                if (!settingsStr || Object.keys(settings).length === 0) {
                    console.log("Attempting to migrate settings from individual KV keys...");
                    ...
                    if (defaultCopyFormat) oldSettings.defaultCopyFormat = defaultCopyFormat;

                    const customImagePrefix = await (IMGBED_KV as any).get(CONFIG_KEYS.customImagePrefix, { type: "text", consistency: "strong" });
                    if (customImagePrefix !== null) oldSettings.customImagePrefix = customImagePrefix;
                    ...
                    if (enableHotlinkProtectionStr) oldSettings.enableHotlinkProtection = enableHotlinkProtectionStr === 'true';

                    const allowedDomainsStr = await (IMGBED_KV as any).get(CONFIG_KEYS.allowedDomains, { type: "text", consistency: "strong" });
                    if (allowedDomainsStr) {
                        try { oldSettings.allowedDomains = JSON.parse(allowedDomainsStr) as string[]; } catch (e) { /* ignore parsing error */ }
                    }

                    const siteDomain = await (IMGBED_KV as any).get(CONFIG_KEYS.siteDomain, { type: "text", consistency: "strong" });
                    if (siteDomain !== null) oldSettings.siteDomain = siteDomain;
                    ...
                    if (Object.keys(oldSettings).length > 0) {
                        ...
                        try {
                            await IMGBED_KV.put(APP_SETTINGS_KEY, JSON.stringify(settings));
                            console.log("Successfully migrated settings to new key:", APP_SETTINGS_KEY);
                            ...
                            console.error("Failed to save migrated settings to new key:", e);
                        }
                    }
                }
            ...
            try {
                ...
                if (settingsStr) {
                    try {
                        ...
                        console.error(`Failed to parse current app settings from KV (${APP_SETTINGS_KEY}) for update:`, e);
                ...
                if (Object.prototype.hasOwnProperty.call(newSettingsToUpdate, 'defaultCopyFormat')) {
                    updatedSettings.defaultCopyFormat = newSettingsToUpdate.defaultCopyFormat;
                }
                if (Object.prototype.hasOwnProperty.call(newSettingsToUpdate, 'customImagePrefix')) {
                    updatedSettings.customImagePrefix = (newSettingsToUpdate.customImagePrefix || "").trim();
                }
                if (Object.prototype.hasOwnProperty.call(newSettingsToUpdate, 'enableHotlinkProtection')) {
                    updatedSettings.enableHotlinkProtection = newSettingsToUpdate.enableHotlinkProtection;
                }
                if (Object.prototype.hasOwnProperty.call(newSettingsToUpdate, 'allowedDomains')) {
                    updatedSettings.allowedDomains = (newSettingsToUpdate.allowedDomains || []).map(d => String(d).trim()).filter(d => d.length > 0);
                }
                if (Object.prototype.hasOwnProperty.call(newSettingsToUpdate, 'siteDomain')) {
                    updatedSettings.siteDomain = (newSettingsToUpdate.siteDomain || "").trim();
                }
                if (Object.prototype.hasOwnProperty.call(newSettingsToUpdate, 'convertToWebP')) {
                    updatedSettings.convertToWebP = newSettingsToUpdate.convertToWebP;
                }

                // 3. Save updated settings object
                await IMGBED_KV.put(APP_SETTINGS_KEY, JSON.stringify(updatedSettings));
