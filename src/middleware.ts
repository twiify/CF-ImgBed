...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  const { action: actionInfo, setActionResult, serializeActionResult } = getActionContext(context);

  if (request.method === "POST" && actionInfo?.name && actionInfo.calledFrom === 'form') {
  ...
  if (sessionIdFromCookie && IMGBED_KV) {
    ...
    console.error('[Middleware] IMGBED_KV not available, cannot validate session.');
  }
  ...
  if (isProtectedRoute && !user) {
    if (currentPath === '/login') { // Should not happen if /login is not in PROTECTED_ROUTES
      return next();
    }
