...
const ApiKeysManager: FunctionalComponent = () => {
  ...
  const showConfirm = useCallback((title: string, message: string, confirmBtnText: string = "确认", cancelBtnText: string = "取消"): Promise<boolean> => {
  ...
  const showPrompt = useCallback((title: string, message: string, initialValue: string = "", placeholder: string = "例如：My Test Key", confirmBtnText: string = "生成", cancelBtnText: string = "取消"): Promise<string | null> => {
    return new Promise((resolve) => {
      setPromptModal({
        isOpen: true, title, message, initialValue, inputPlaceholder: placeholder,
        confirmText: confirmBtnText,
        cancelText: cancelBtnText,
  ...
  const fetchApiKeys = useCallback(async () => {
    ...
    try {
      ...
      const validKeys = (result.data || []).filter(key => typeof key.id === 'string');
  ...
  const executeRevokeAction = useCallback(async (keyToExecute: DisplayApiKey | null) => {
    if (!keyToExecute) {
      return;
    }
  ...
  }, [showAlert, fetchApiKeys]); // Dependencies are stable callbacks and setters

  const handleOpenRevokeModal = useCallback((key: DisplayApiKey) => {
  ...
  }, [showConfirm, executeRevokeAction]); // Depends on executeRevokeAction
