...
const ImageManager: FunctionalComponent = () => {
    ...
    const fetchDirectoryContents = useCallback(
        async (directoryPath: string = '') => {
            ...
            try {
                const result = await actions.image.listDirectoryContents({
                    path: directoryPath,
                });
                if (result.error) {
                ...
                if (result.data) {
                    ...
                    setCurrentDirectoryTotalSize(
                        result.data.currentDirectoryTotalSize || undefined,
                    );
                }
            } catch (error: any) {
    ...
    const selectedImageIds = useMemo(
        () =>
            images
                .filter((img) => selectedItems.has(img.id))
                .map((img) => img.id),
    ...
    const allCurrentlySelected = useMemo(() => {
        ...
        return (
            ...
            allImageIds.every((id) => selectedItems.has(id))
        );
    }, [images, selectedItems]);
    ...
    const deleteItems = useCallback(
        async (imageIds: string[], isSingle: boolean) => {
            ...
            try {
                const result = await actions.image.deleteImagesAction({
                    imageIds,
                });
                if (result.error) {
                    throw new Error(result.error.message);
    ...
    const promptForDirectoryAndMove = useCallback(
        async (imageIds: string[], _isSingle: boolean) => {
            if (imageIds.length === 0) return;
                                ...
                                {images.map((image, index) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            style={{
                                                animationDelay: `${(directories.length + index) * 50}ms`,
                                            }}
                                        >
                                            {/* Checkbox */}
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <div className="flex items-center gap-1">
                                                    <span
                                                        className="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-mono font-medium bg-surface border border-border-light text-text-secondary truncate"
                                                        title={escapeHtml(
                                                            image.id,
                                                        )}
                                                    >
